...
// car reference speed in MPH (miles per hour)
double ref_v = 100;

// weight for each cost function term
double w_cte   = 1000;
double w_epsi  = 1000;
double w_v     = 1;
double w_delta = 50;
double w_a     = 50;
double w_delta_d = 250000;
double w_a_d   = 5000;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Minimize the use of actuators for steering (delta) and acceleration (a)/[]
    for (int t = 0; t < N - 1; t++) {
    ...
    for (int t = 0; t < N - 2; t++) {
      fg[0] += w_delta_d * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
      fg[0] += w_a_d     * CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);
    }
